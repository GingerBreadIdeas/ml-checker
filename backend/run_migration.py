...
def run_migration():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if result.fetchone():
                print(
                    "✅ Metrics column already exists in chat_messages table"
                )
                return True

            # Add the metrics column
            conn.execute(
                text("ALTER TABLE chat_messages ADD COLUMN metrics JSONB")
            )
            conn.commit()

            print(
                "✅ Successfully added metrics column to chat_messages table"
            )
            return True
