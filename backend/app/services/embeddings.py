...
def reduce_to_2d(embeddings: np.ndarray, random_state: int = 42) -> np.ndarray:
    ...
    """
    tsne = TSNE(n_components=2, random_state=random_state, perplexity=min(30, max(5, len(embeddings) // 10)))
...
def create_2d_embeddings_for_texts(texts: List[str]) -> Tuple[np.ndarray, np.ndarray]:
    ...
    reduced_embeddings = reduce_to_2d(embeddings) if len(texts) > 1 else np.zeros((1, 2))
