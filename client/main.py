...
from client import CheckerCallbackHandler
...
def main():
    ...
    if not cat_facts_file.exists():
        try:
            response = requests.get("https://huggingface.co/ngxson/demo_simple_rag_py/raw/main/cat-facts.txt", timeout=10)
    ...
    try:
        ...
        documents = [fact.strip() for fact in content.split('\n') if fact.strip()]
    ...
    mlchecker_handler = CallbackHandler()
    ...
    for i, doc_text in enumerate(documents):
        ...
        for j, chunk in enumerate(chunks):
            doc_chunks.append(
                Document(
                    page_content=chunk,
                    metadata={"doc_id": i, "chunk_id": j, "source": f"document_{i}"}
                )
            )
